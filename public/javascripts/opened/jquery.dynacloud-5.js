...
jQuery.fn.dynaCloud = function(outElement) {
 ...
 return this.each(function() {
  ...
  if (jQuery.wordStats && jQuery.dynaCloud.wordStats) {
   ...
   // var elems = jQuery(this).text().replace("&nbsp;", ' ').replace(/[^A-Z\xC4\xD6\xDCa-z\xE4\xF6\xFC\xDF0-9_]/g, ' ').replace(jQuery.dynaCloud.stopwords, ' ').split(' ');
   var elems = jQuery(this).text().replace(/\s+/g, ' ').replace("&nbsp;", ' ').replace(jQuery.dynaCloud.stopwords, ' ').split(' ');
  ...
  for (var i = 0; i < l; ++i) {
	...
	if (cl[i].count == 1) {
		...
		size = Math.ceil((cl[i].count / max) * jQuery.dynaCloud.scale);
	}
   out.append('<a href="#' + cl[i].el + '" style="font-size: ' + size + 'em"><span>' + cl[i].el + '</span></a> &nbsp; ');
  }
