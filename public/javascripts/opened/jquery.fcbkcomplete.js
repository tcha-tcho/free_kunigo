	    ...
	    $.fn.fcbkcomplete = function (opt) 
	    {
    		return this.each(function()
			{
		        ...
		        {	
					if (value.match(',') != null){
						...
						for (var i=0; i < values.length; i++) {
							...
							if (uniqueness(clean_value)) {
								ready_values.push(clean_value);
							};
						};
						for (var i=0; i < ready_values.length; i++) {
							var skip = true
							if(ready_values.length == (parseInt(i)+1)){
								skip = false
							};
							addEachItem(ready_values[i], ready_values[i], preadded,skip);
						};
						
					} else {
						clean_unique_value = title.replace(/^\s*|\s*$/g,"");//trim
						if (uniqueness(clean_unique_value)) {
							addEachItem(clean_unique_value, value, preadded,false);
						}
					}
					...
					function addEachItem(ititle, ivalue, ipreadded,skip_func){
	                    ...
	                    function(event)
	                    {
	                        ...
	                        if (event.keyCode != 40 && event.keyCode != 38 && etext.length != 0) 
	                        {
	                            ...
	                            if (options.json_url) 
	                            {
	                                ...
	                                {
										if (etext.length > options.lazy_get && etext.indexOf(',') == -1)
										{
